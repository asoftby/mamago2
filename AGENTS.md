...
CRITICAL: Before modifying the repository, run `sh scripts/git/session-start-gate.sh`. Repository work may start only when it reports that local `HEAD` exactly matches fresh `origin/dev`. If it reports stale, ahead, diverged, or tracked/staged work-in-progress, STOP and reconcile deliberately. Never use `reset --hard`, force-push, broad checkout/restore, or an automatic pull/rebase to make the gate green.
