...
export interface OfferMediaSyncInput { offerId: string; ownerUserId: string; attachmentIds: readonly number[]; mediaPolicy: MediaPolicyName; sourceRecordKey: string }
...
export class OfferMediaSyncer {
  ...
  async sync(input: OfferMediaSyncInput): Promise<OfferMediaSyncResult> {
    ...
    const result = await this.fullDelegate.sync({ offerId: input.offerId, ownerUserId: input.ownerUserId, attachmentIds, sourceRecordKey: input.sourceRecordKey });
