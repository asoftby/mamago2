...
import { collapseOfferPlaceRelations, OFFER_PLACE_RELATION_COLLAPSE_POLICY_VERSION, type OfferPlaceRelationCollapseResult } from "./collapseOfferPlaceRelations";
...
export function buildOfferCanonicalCommitHash(input: { candidate: NormalizedOfferCandidate; context: OfferCommitContext; draft: OfferCreateDraft; collapse: OfferPlaceRelationCollapseResult }): string {
  const material = {
    ...
    ...(input.collapse.status === "RESOLVED" && input.collapse.sourceRowCount > 1 ? { relationCollapse: { policyVersion: OFFER_PLACE_RELATION_COLLAPSE_POLICY_VERSION, effectiveLegacyPlaceId: input.collapse.effectiveLegacyPlaceId, normalizedRelationEvidence: input.collapse.normalizedRelationEvidence, relationKeys: input.collapse.uniqueRelationKeys } } : {}),
  };
...
export function buildOfferCreateDraft(input: { candidate: NormalizedOfferCandidate; context: OfferCommitContext }): OfferCreateDraftResult {
  ...
  return { ok: true, draft, warnings, canonicalHash: buildOfferCanonicalCommitHash({ candidate, context, draft, collapse }) };
}
