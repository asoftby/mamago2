export const PHONE_LABEL_MAX_LENGTH = 40;
...
function normalizePhoneLabel(value: string | null | undefined): string | null {
  ...
  return normalized.slice(0, PHONE_LABEL_MAX_LENGTH);
}
...
export function normalizePhoneFields(input: PhoneFields): Required<PhoneFields> {
...
export function getNormalizedPhones(input: PhoneFields): NormalizedPhone[] {
  const normalized = normalizePhoneFields(input);
  const rawPhones = [
