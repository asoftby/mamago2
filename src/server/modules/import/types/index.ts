...
export interface NormalizedEventImport extends NormalizedImportPayload {
  ...
  categoryCandidates: string[];
  /**
   * Метки источника вне автоматически распознаваемого списка категорий
   * (например ABWS type.id вне CATEGORY_TYPE_IDS — площадки-имена вперемешку
   * с реальными сигналами вроде "Театр кукол"/"Детям"). Не используется для
   * автоматического маппинга — только для ручного ревью: помогает
   * ревьюеру увидеть сигнал источника и вручную выставить категорию/жанр.
   */
  otherCategoryCandidates?: string[];
  /**
