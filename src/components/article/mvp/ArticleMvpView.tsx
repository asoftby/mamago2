...
function activityDescriptor(
  ...
  return {
    blockId: block.id,
    blockType: "activityCard",
    subjectId: `catalog:${block.entityType}:${block.entityId}`,
    subjectTitle: title?.trim() || `${block.entityType} ${block.entityId.slice(0, 8)}`,
    subjectSource: "CATALOG",
    catalogEntityType: block.entityType,
    catalogEntityId: block.entityId,
  };
}
...
function ArticleInlineToc({ branches }: { branches: ArticleTocBranch[] }) {
  ...
  return (
    <nav aria-label="Содержание статьи" className="not-prose max-w-[720px] mb-8 md:mb-10 font-sans">
      <p className="text-[20px] font-bold tracking-tight text-foreground mb-2.5">Содержание</p>
      <ul className="list-none m-0 p-0 space-y-0.5 text-[18px] font-normal leading-snug tracking-tight text-foreground/90">
        {branches.map((branch) => (
          <li key={branch.entry.id}>
            <a href={`#${branch.entry.anchorId}`} className="underline-offset-[3px] hover:underline decoration-foreground/25 hover:decoration-foreground/50">
              {branch.entry.text}
            </a>
            {branch.children.length > 0 ? (
              <ul className="list-none m-0 mt-1.5 space-y-1 pl-4 border-l border-border/50">
                {branch.children.map((c) => (
                  <li key={c.id}>
                    <a href={`#${c.anchorId}`} className="text-muted-foreground underline-offset-[3px] hover:underline hover:text-foreground decoration-transparent hover:decoration-foreground/30">
                      {c.text}
                    </a>
                  </li>
                ))}
...
export function ArticleMvpView({
  ...
  const leadHtml = !isBreakingNews ? deriveArticleLeadHtml({ blocks: blocks as ArticleBlockMvp[] }) : null;
  const leadPlain = deriveArticleLeadPlainText({ blocks: blocks as ArticleBlockMvp[] }) || "";
      ...
      >
            ...
            {groupArticleInfoBlocks(blocks).map((group) => {
              ...
              const body = (() => {
                if (block.type === "text") {
                  ...
                  if (legacyEmbed?.provider === "youtube") return <ArticleEmbedBlock value={block.text} />;
                  return (
                    <div
                      className="font-serif leading-[1.75] md:leading-[1.8] text-foreground/95 mb-6 md:mb-7 last:mb-0 [&_p]:mb-4 [&_p:last-child]:mb-0 [&_ul]:my-3 [&_ol]:my-3 [&_ul]:list-disc [&_ul]:pl-6 [&_ol]:list-decimal [&_ol]:pl-6 [&_li]:my-0.5 [&_a]:text-primary [&_a]:underline [&_a]:underline-offset-2 [&_a]:decoration-primary/40 [&_code]:font-mono [&_code]:text-[13px] [&_code]:text-muted-foreground"
                ...
                if (block.type === "callout") return <ArticleCalloutBlock variant={block.variant} title={block.title} text={block.text} />;
                if (block.type === "heading") {
                  ...
                  return <Tag id={articleHeadingAnchorId(block.id)} className={cls}>{block.text}</Tag>;
                }
                if (block.type === "image") {
                  return (
                ...
                if (block.type === "activityCard") {
                  ...
                  const wrap = (node: ReactNode) => (
                    <ArticlePerformanceScope blocks={[activityDescriptor(block, cardTitle)]} mode="card">
                      {node}
                    </ArticlePerformanceScope>
                  );
                  if (block.entityType === "PLACE") {
                    if (!c || c.kind !== "place-live") return null;
                    return wrap(<ArticleLivePlaceBlock card={c} />);
                  }
                  if (!c) return <p className="text-sm text-muted-foreground my-6">Карточка: сущность не найдена</p>;
                  if (block.entityType === "EVENT" && c.kind === "basic") {
                    return wrap(
                      <ArticleEventCardBlock title={c.title} href={c.href} dateLabel={c.meta ?? ""} location="" ageLabel="" badge="" />,
                    );
                  }
                  if (c.kind === "offer-embed") return wrap(<ArticleOfferEmbed card={c} />);
                  if (c.kind === "place-embed" || c.kind === "place-live") return null;
                  return wrap(<ArticleOfferCardBlock title={c.title} href={c.href} image={c.imageUrl ?? undefined} />);
                }
                return null;
              })();
        ...
        {showJournalFooter ? (
          <footer className="mt-14 md:mt-16 pt-8 border-t border-border/60 text-sm text-muted-foreground">
            <Link href={footerHref} className="text-primary hover:underline underline-offset-2">← Все материалы</Link>
          </footer>
        ) : null}
      </article>
    </div>
