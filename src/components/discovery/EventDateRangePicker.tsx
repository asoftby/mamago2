...
export function EventDateRangePicker({ applied, onApply }: { applied: DiscoveryFilters; onApply: (patch: Partial<DiscoveryFilters>) => void }) {
  ...
  const resetDateRange = () => {
    dispatch({ type: "reset" });
    onApply({ whenPreset: null, dateFrom: null, dateTo: null });
  };
  ...
  if (!mobile) return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="start" className="w-[650px] p-5">{calendar(2)}<div className="mt-3 flex items-center justify-between gap-4"><MobileOverlayResetAction disabled={!draft.from} onClick={resetDateRange} /><Button size="sm" disabled={!draft.from} onClick={() => { onApply({ whenPreset: null, dateFrom: draft.from, dateTo: draft.to }); setOpen(false); }}>Применить</Button></div></PopoverContent>
    </Popover>
  );
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="bottom" className="flex h-[92dvh] flex-col gap-0 rounded-t-3xl bg-white p-0">
        ...
        <div className="sticky bottom-0 border-t bg-white p-4 pb-[calc(1rem+env(safe-area-inset-bottom))]"><div className="flex items-center gap-3"><MobileOverlayResetAction disabled={!draft.from} onClick={resetDateRange} /><div className="min-w-0 flex-1" /><Button className="min-w-[10rem] shrink-0 rounded-full" disabled={!draft.from} onClick={() => { onApply({ whenPreset: null, dateFrom: draft.from, dateTo: draft.to }); setOpen(false); }}>Показать {resultCount ?? "…"} событий</Button></div></div>
      </SheetContent>
    </Sheet>
  );
}
