...
export function AddChildSheet({ isOpen, onClose, childData }: AddChildSheetProps) {
  ...
  useEffect(() => {
    if (isOpen && childData) {
      ...
      setSelectedInterests(childData.systemInterests?.map(i => i.interestSlug) || []);
      setCustomInterests(childData.customInterests?.map(i => i.label) || []);
    }
  }, [isOpen, childData]);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Close sheet and refresh
      console.log("Closing sheet and refreshing page");
      onClose();
      router.refresh();
    } catch (err) {
            ...
            <div className="flex flex-wrap gap-2">
              {SYSTEM_INTERESTS.map((interest) => (
                <Badge
                  ...
                  className={cn(
                    "cursor-pointer transition-colors h-9 px-4 text-sm",
              ...
              <div className="flex flex-wrap gap-2">
                {customInterests.map((interest) => (
                  <Badge
                    key={interest}
                    variant="secondary"
                    className="h-9 px-4 pr-2 text-sm"
                  >
                    {interest}
                    <button
                      type="button"
