...
interface ProfileSignalPickerProps {
...
/**
 * Пикер PROFILE-сигналов для формы профиля пользователя.
 *
 * Правила:
 * - Показывает только сигналы с domain=PROFILE
 ...
 */
export function ProfileSignalPicker({
  value,
  onChange,
  disabled = false,
}: ProfileSignalPickerProps) {
  ...
  useEffect(() => {
    ...
    fetch("/api/public/signals/profile?includeDeprecated=true")
      ...
      .catch((err) => {
        if (!cancelled) {
          console.error("[ProfileSignalPicker] Failed to load signals:", err);
          setGroups([]);
  ...
  if (error) {
    console.warn(`[ProfileSignalPicker] Error loading signals: ${error}`);
