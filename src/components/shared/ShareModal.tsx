...
const TelegramIcon = ({ size = 20 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <path d="M21.8 2.6 2.3 10.1c-1.3.5-1.3 1.3-.2 1.6l4.8 1.5 11.1-7c.5-.3 1 0 .6.4l-9 8.1-.3 5c.4 0 .9-.2 1.2-.5l2.8-2.7 5 3.7c.9.5 1.5.2 1.8-.8L23.2 4c.3-1.3-.5-1.9-1.4-1.4z" fill="currentColor" />
  </svg>
);
...
export type ShareAnalyticsChannel = "telegram" | "whatsapp" | "copy" | "native";

const PLATFORMS = [
  {
    id: "tg",
    analyticsChannel: "telegram" as const,
    label: "Telegram",
  ...
  {
    id: "wa",
    analyticsChannel: "whatsapp" as const,
    label: "WhatsApp",
...
export type ShareModalProps = {
  ...
  subtitle?: string;
  entityNoun?: string;
  /** Fires only after a concrete share/copy action, never for opening the modal. */
  onShare?: (channel: ShareAnalyticsChannel) => void;
};
...
function ShareContent({
  ...
  const handleCopy = async () => {
    try {
      ...
      // Share analytics is best-effort and never changes the UX path.
    }
  };
  ...
  const handleNativeShare = async () => {
    try {
      ...
      // User cancelled or the platform rejected the native share.
    }
  };
  ...
  return (
    <div style={{ padding: "26px 26px 24px" }}>
      ...
      <p style={{ margin: "0 0 22px", fontSize: 13, color: T.ink3, lineHeight: 1.5 }}>
        ...
        {PLATFORMS.map((p) => {
          ...
          return (
            <a
              ...
              onClick={() => onShare?.(p.analyticsChannel)}
            ...
            >
              ...
              <span style={{ flex: 1, fontSize: 15, fontWeight: 600, color: T.ink, letterSpacing: "-.01em" }}>
                {p.label}
              </span>
      ...
      <button
        ...
        onMouseEnter={(e) => { if (!copied) e.currentTarget.style.borderColor = T.ink; }}
        onMouseLeave={(e) => { e.currentTarget.style.borderColor = copied ? "rgba(31,138,91,.35)" : T.line; }}
      >
        <span
          style={{
            width: 40,
        ...
        <span style={{ flex: 1, minWidth: 0 }}>
          <span style={{ display: "block", fontSize: 15, fontWeight: 600, color: copied ? "#1A7A4A" : T.ink, letterSpacing: "-.01em" }}>
            {copied ? "Скопировано!" : "Копировать ссылку"}
          </span>
        </span>
        {!copied && <span style={{ fontSize: 16, color: T.ink3 }}>→</span>}
      </button>
      ...
      {hasNativeShare && (
        ...
        >
          ...
          <span style={{ flex: 1 }}>
            <span style={{ display: "block", fontSize: 15, fontWeight: 600, color: T.ink, letterSpacing: "-.01em" }}>
              Поделиться через…
            </span>
            <span style={{ display: "block", fontSize: 12, color: T.ink3, marginTop: 2 }}>Другие приложения</span>
          </span>
          <span style={{ color: T.ink3, fontSize: 16 }}>→</span>
        </button>
      )}
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent
          ...
          style={{
            ...
            border: `1px solid ${T.line}`,
            background: "#FFFFFF",
            boxShadow: "0 2px 0 rgba(255,255,255,.5) inset, 0 40px 80px -20px rgba(20,18,16,.22)",
          }}
        >
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "center", padding: "14px 0 0" }}>
          <span style={{ width: 36, height: 4, borderRadius: 99, background: T.line2, display: "block" }} />
        </div>
        {innerContent}
