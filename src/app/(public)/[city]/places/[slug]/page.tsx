...
import { findPlaceBySlug, findPlaceBySlugInCity } from "@/lib/slug/placeSlugService";
 ...
 *
 * `Place.slug` is only unique per-city (`@@unique([cityId, slug])`), so
 * this route resolves the place through `findPlaceBySlugInCity()`
 * (city-scoped) rather than the global `findPlaceBySlug()` — closing the
 * cross-city collision risk the pre-city-scoped `/places/{slug}` canonical
 * had (see `docs/migration/seo/final-url-architecture-2026-08-15.md` §2,
 * BACKLOG-115). The global lookup is still used, but only as a
 * city-correcting redirect when a slug exists under a *different* city
 * than the one requested — it never renders content for a mismatched city.
 */
...
export default async function PlacePage({ params }: PlacePageProps) {
  ...
  if (isLegacyId) {
    // Legacy ID - find by id and redirect to the canonical slug URL, under
    // the place's own city (never assume it matches the URL's city).
    const legacyPlace = await prisma.place.findUnique({
      where: { id: slug },
      select: { id: true, slug: true, cityId: true, city: { select: { slug: true } } },
    });

    if (!legacyPlace) notFound();

    placeId = legacyPlace.id;
    if (legacyPlace.slug && legacyPlace.city?.slug) {
      permanentRedirect(buildCityPublicPath({ citySlug: legacyPlace.city.slug, type: "place", slug: legacyPlace.slug }));
    }
    // No city on this legacy Place — cannot build a city-scoped canonical.
    // Fall through and let the "wrong/no city" cityId check below 404, so
    // this doesn't silently render under an arbitrary city segment.
    if (legacyPlace.cityId !== city.id) notFound();
  } else {
    // City-scoped lookup first — the collision-safe path (BACKLOG-115).
    const slugResult = await findPlaceBySlugInCity(city.id, slug);

    if (slugResult) {
      placeId = slugResult.placeId;
      if (slugResult.isRedirect) {
        const currentPlace = await prisma.place.findUnique({ where: { id: placeId }, select: { slug: true } });
        if (!currentPlace?.slug) notFound();
        permanentRedirect(buildCityPublicPath({ citySlug: city.slug, type: "place", slug: currentPlace.slug }));
      }
    } else {
      // Not found under this city — check whether the slug belongs to a
      // *different* city and redirect there instead of 404ing (e.g. a
      // wrong city segment in an old link). Never renders content here;
      // only ever a redirect or a genuine not-found.
      const globalResult = await findPlaceBySlug(slug);
      if (!globalResult) notFound();
      const otherPlace = await prisma.place.findUnique({
        where: { id: globalResult.placeId },
        select: { slug: true, city: { select: { slug: true } } },
      });
      if (!otherPlace?.slug || !otherPlace.city?.slug) notFound();
  ...
  const relatedPlaces = relatedPlacesRaw
    ...
    // A related Place (same placeGroupId, e.g. a chain's other branches) is
    // not guaranteed to be in the same city — link to its own city, and
    // drop it entirely if it somehow has none (can't build a valid href).
    .filter((relatedPlace) => Boolean(relatedPlace.city?.slug))
    .map((relatedPlace) => ({
      id: relatedPlace.id,
      title: relatedPlace.title,
      href: buildCityPublicPath({ citySlug: relatedPlace.city!.slug, type: "place", slug: relatedPlace.slug || relatedPlace.id }),
      category:
  ...
  // truth, no "minsk" fallback needed.
  const placeCitySlug = city.slug;
  ...
  const breadcrumbItems: Array<{ label: string; href?: string }> = [
    { label: "Главная", href: "/" },
    { label: place.city?.name ?? placeCitySlug, href: `/${placeCitySlug}` },
    { label: displayTitle },
  ];
  const canonicalPath = buildCityPublicPath({ citySlug: placeCitySlug, type: "place", slug: place.slug ?? place.id });
  ...
  const breadcrumbJsonLd = buildBreadcrumbJsonLd(
    [
      { name: "Главная", path: "/" },
      { name: place.city?.name ?? placeCitySlug, path: `/${placeCitySlug}` },
