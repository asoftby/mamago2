...
import { ArticleCategoryNav } from "@/components/article/ArticleCategoryHub";
...
import { listPublicArticleCategories } from "@/server/article/articleCategoryHub";
...
export default async function CityBlogPage({ params, searchParams }: PageProps) {
  ...
  const [journal, categories] = await Promise.all([
    ...
    listPublicArticleCategories(city),
  ]);
  ...
  return (
    <main>
      <ArticleCategoryNav categories={categories} citySlug={city.slug} />
