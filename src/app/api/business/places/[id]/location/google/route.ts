 ...
 * Set location from Google Places API
 */
...
export async function POST(
  ...
  try {
    ...
    if (!googlePlaceId || lat === undefined || lng === undefined) {
      return NextResponse.json(
        { 
          ...
          message: "googlePlaceId, lat, and lng are required" 
        },
        { status: 400 }
      );
    }
    ...
    const updatedPlace = await updatePlaceLocation(id, {
      ...
      lng: parseFloat(lng),
      googlePlaceId,
      formattedAddr: formattedAddr || null,
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      if (error.code === "P2002") {
        ...
        console.error("[place-location-google] Unique constraint violation:", fields);
