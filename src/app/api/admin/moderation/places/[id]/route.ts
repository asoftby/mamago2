 ...
 * Notifications are sent inside the service layer — not here.
...
export async function POST(
  ...
  try {
    ...
    try {
      switch (action) {
        case "APPROVE":       await approvePlace(id, user.id, comment); break;
        case "NEEDS_REVISION": await needsRevisionPlace(id, user.id, comment); break;
        case "REJECT":        await rejectPlace(id, user.id, comment); break;
      }
    } catch (serviceError) {
      return NextResponse.json(
    ...
    console.error("Place moderation error:", error);
