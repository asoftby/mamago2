...
import { promoteToAdminByEmail } from "@/server/services/userRole.service";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Promote user
    const result = await promoteToAdminByEmail(email, user.id);
    ...
    return NextResponse.json({
      ...
      message: result.wasAlreadyAdmin
        ? `Пользователь ${email} уже является ADMIN`
        : `Пользователь ${email} успешно повышен до ADMIN`,
    });
  } catch (error) {
