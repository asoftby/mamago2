...
import { getAnalyticsFunnels } from "@/server/services/analytics/analyticsFunnels.service";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getAnalyticsFunnels(filters);
    ...
    console.error("GET /api/admin/analytics/funnels:", error);
    return NextResponse.json(
      { error: msg || "Failed to load funnel analytics" },
      { status: 500 },
    );
  }
}
