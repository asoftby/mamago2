...
export async function StoriesManagementPanel({ searchParams }: Props) {
  ...
  return <div className="space-y-6">
    ...
    <section className="space-y-3"><h2 className="text-lg font-semibold">Кандидаты для ручного добавления</h2><p className="text-sm text-gray-500">Поиск ограничен выбранными городом, датой и первыми 50 occurrence. Offer автоматически не включаются.</p>
      {candidates.length === 0 ? <div className="rounded-xl border border-dashed p-5 text-gray-500">Кандидатов нет.</div> : <div className="grid gap-2">{candidates.map((session) => <article key={session.id} className="flex items-center justify-between gap-3 rounded-xl border bg-white p-3"><div><div className="font-medium">{session.offer.title}</div><div className="text-sm text-gray-500">{session.offer.place.title} · {fmt(session.startAt)}</div><div className="text-xs text-amber-700">Только вручную · источник Offer</div></div><form action={addOfferOccurrenceAction}><input type="hidden" name="cityId" value={cityId}/><input type="hidden" name="date" value={date}/><input type="hidden" name="sessionId" value={session.id}/><button className="rounded-lg bg-emerald-600 px-3 py-2 text-sm text-white">Добавить в Stories</button></form></article>)}</div>}
    </section>
