...
## [BACKLOG-130] Extend UserEvent taxonomy meta (categorySlug) to remaining CARD_VIEW producers (ActivityMock-fed EVENT/OFFER grids), and resolve OFFER/ROUTE taxonomy gap

- Status: OPEN
- Priority: P1
- Area: Analytics / Discovery / Offers / Routes
- Added: 2026-08-25
- Reason deferred: PROD taxonomy-meta audit found 0% `categorySlug` coverage on `UserEvent`, blocking `UserBehaviorProfile.preferredCategories` rebuild. Two follow-up passes (same branch, base `e8222bb9`) closed DETAIL_OPEN/SAVE/PLAN_ADD for EVENT/PLACE/ARTICLE, then ARTICLE CARD_VIEW (`CityHomeContentRows`, `BlogIndex`, the continuous reader's `article_impression`) — all zero-additional-query. What's left (EVENT/OFFER CARD_VIEW on the discovery/city-home grids) needs a wider type change, not a local select widen, so it stayed out of the minimal-fix scope (`не делай большой refactor`).
- Context: canonical taxonomy source is `EventCategory` (`publicationType` ∈ EVENT/PLACE/OFFER/ROUTE/ARTICLE), but only EVENT (`Activity.eventCategoryId`), PLACE (`Place.primaryCategoryId` — a legacy free-text `Place.category` string field also exists and must NOT be used), and ARTICLE (`Article.categoryId`) actually have a populated FK into it. `Offer.category` is the unrelated `PartyCategory` enum (VENUE/ANIMATOR/SHOW/…) — mapping it into `categorySlug` would conflate two different classification systems. `Route` has no taxonomy field in the schema at all (checked directly — only `ageTags`/`budgetLevel`/`agePolicy`).
- Current state: `src/lib/analytics/types.ts` declares `AnalyticsMetaPayload.categorySlug` as the single canonical field. Enriched (zero additional DB query in all cases): EVENT/PLACE/ARTICLE DETAIL_OPEN, SAVE, PLAN_ADD; ARTICLE CARD_VIEW in `CityHomeContentRows.tsx` (journal row), `BlogIndex.tsx` (featured + list), and `ContinuousArticleReader.tsx`'s `article_impression` (categorySlug threaded via a new `data-article-category` DOM attribute, sourced from `PublicContinuousArticleDto.section.slug`, already selected by `nextArticleInSection.ts`/`buildContinuousArticleSeed.ts`). Still unenriched: `DiscoveryActivitiesGrid.tsx` (EVENT/OFFER dynamic), `CityHomeContentRows.tsx`'s "classes" row (OFFER), `[city]/programs/page.tsx` (OFFER) — all three consume the shared `ActivityMock` type (`src/types/activity.ts`), which has no category field of any kind (checked directly); the underlying feed query(-ies) that populate `ActivityMock[]` were not traced (multiple discovery/city-home data sources, not one shared loader). `ArticleTelegramInvitation.tsx`'s CARD_VIEW (`articleEvent: "article_telegram_cta_impression"`) was deliberately left unenriched too — it is structurally excluded from `totalViews`/`preferredCategories` by the reducer's `isNonContentCardImpression` check regardless of meta content, so enriching it would be inert. `RouteCard.tsx` (ROUTE) and OFFER producers remain permanently unenriched pending a product decision, not just engineering.
- Dependencies: for EVENT/OFFER CARD_VIEW — requires locating and auditing the actual feed query/queries behind `ActivityMock[]` for `DiscoveryActivitiesGrid`/`CityHomeContentRows`/`programs page` (not yet identified in this session), then deciding whether to extend the shared `ActivityMock` type (affects every consumer, not just analytics) or introduce a narrower analytics-only sibling type. OFFER needs a product decision: introduce a separate `partyCategory` meta field (different namespace, not `categorySlug`) or leave PartyCategory out of preference-signal analytics entirely. ROUTE needs a product decision on whether routes should get a taxonomy field at all (schema change) before any analytics work is possible.
- Acceptance criteria: for EVENT/OFFER CARD_VIEW — feed query(-ies) identified and audited; if category is already joined, select widened (zero new query); if extending `ActivityMock` is judged too broad, an analytics-only narrower type is threaded instead — either way no lookup-per-card-render. For OFFER/ROUTE — an explicit product/engineering decision recorded here (or a new backlog entry) before any code changes; no `entityType`-as-category or cross-taxonomy-system guessing.
- Source: Analytics Contract v1 — PROD taxonomy root-cause audit, base `e8222bb9`, 2026-08-25; CARD_VIEW closure pass, same base, 2026-08-25
