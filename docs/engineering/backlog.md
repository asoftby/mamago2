...
## [BACKLOG-130] OFFER/ROUTE UserEvent taxonomy meta gap (product decision needed)

- Status: OPEN
- Priority: P1
- Area: Analytics / Offers / Routes
- Added: 2026-08-25
- Reason deferred: PROD taxonomy-meta audit found 0% `categorySlug` coverage on `UserEvent`, blocking `UserBehaviorProfile.preferredCategories` rebuild. Three follow-up passes (same branch `fix/analytics-taxonomy-enrichment`, base `e8222bb9`) closed it for every entity type that has a compatible taxonomy source: EVENT/PLACE/ARTICLE DETAIL_OPEN+SAVE+PLAN_ADD, ARTICLE CARD_VIEW, then EVENT CARD_VIEW (`DiscoveryActivitiesGrid`, via `ActivityMock.eventCategorySlug` — turned out to be a 5-file zero-query fix, not the broad-type-change originally feared). What remains — OFFER and ROUTE — is not an engineering scoping problem anymore, it's a product/taxonomy-design one: neither entity has data compatible with `EventCategory.slug`.
- Context: canonical taxonomy source is `EventCategory` (`publicationType` ∈ EVENT/PLACE/OFFER/ROUTE/ARTICLE — the enum itself reserves an OFFER/ROUTE slot that is unused in practice). `Offer.category` is the unrelated `PartyCategory` enum (VENUE/ANIMATOR/SHOW/…) — mapping it into `categorySlug` would conflate two different classification systems. `Route` has no taxonomy field in the schema at all (checked directly — only `ageTags`/`budgetLevel`/`agePolicy`).
- Current state: `src/lib/analytics/types.ts` declares `AnalyticsMetaPayload.categorySlug` as the single canonical field, populated with zero additional DB queries for: EVENT/PLACE/ARTICLE DETAIL_OPEN, SAVE, PLAN_ADD; ARTICLE CARD_VIEW (`CityHomeContentRows.tsx` journal row, `BlogIndex.tsx` featured+list, `ContinuousArticleReader.tsx`'s `article_impression`); EVENT CARD_VIEW (`DiscoveryActivitiesGrid.tsx`, via `ActivityMock.eventCategorySlug` populated in `mapDiscoveryEventToActivityMock.ts` from the already-`include`d `eventCategory` relation in `kudaDiscoveryFeed.ts`/`loadUpcomingPlaceEvents.ts`). Only genuinely-unfillable gaps remain: `CityHomeContentRows.tsx`'s "classes" row (OFFER), `[city]/programs/page.tsx` (OFFER), `RouteCard.tsx` (ROUTE) — no amount of select-widening fixes these, the source data literally isn't `EventCategory`-shaped. `ArticleTelegramInvitation.tsx`'s CARD_VIEW (`articleEvent: "article_telegram_cta_impression"`) also stays unenriched — structurally excluded from `totalViews`/`preferredCategories` by the reducer's `isNonContentCardImpression` check regardless of meta content, so enriching it would be inert (not a taxonomy-gap issue, noted here only so it isn't mistaken for one).
- Dependencies: OFFER needs a product decision: introduce a separate `partyCategory` meta field (different namespace, not `categorySlug`) or leave PartyCategory out of preference-signal analytics entirely. ROUTE needs a product decision on whether routes should get a taxonomy field at all (schema change) before any analytics work is possible.
- Acceptance criteria: an explicit product/engineering decision recorded here (or a new backlog entry) for OFFER and for ROUTE before any code changes; no `entityType`-as-category or cross-taxonomy-system guessing regardless of which way the decision goes.
- Source: Analytics Contract v1 — PROD taxonomy root-cause audit, base `e8222bb9`, 2026-08-25; ARTICLE CARD_VIEW closure pass, same base, 2026-08-25; EVENT CARD_VIEW closure pass, same base, 2026-08-25
