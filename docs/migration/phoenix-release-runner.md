...
## Tool-generated live-state checkpoint

A live-state checkpoint closes a boundary that immutable progress reports
cannot represent without modifying or synthesizing a failure report. It is
generated by the fixed image only after validating both pinned recovery
reports, the approved manifest, the protected Place supplemental manifest and
files, and the exact live lineage/target state. It is not a manually authored
report and it is not a generic force/resume mechanism.

Generation is a `--plan` operation. Its database client exposes reads only;
the sole write is an atomic, mode-0600 checkpoint at the requested new path,
...
pnpm migration:phoenix-release \
  ...
  --manifest docs/migration/releases/phoenix-approved-2026-07-30.json \
  --plan \
  --create-live-checkpoint <new checkpoint path> \
  --checkpoint-anchor-report <immutable 2dc report> \
  --checkpoint-anchor-report-sha256 <exact SHA-256> \
  --checkpoint-anchor-report-code-sha <exact code SHA> \
  --checkpoint-partial-report <immutable 4ff report> \
  --checkpoint-partial-report-sha256 <exact SHA-256> \
  --checkpoint-partial-report-code-sha <exact code SHA> \
  --checkpoint-protected-manifest <supplemental manifest path> \
  --checkpoint-protected-manifest-sha256 <exact SHA-256>
```

The checkpoint records only safe identity hashes, counts, target IDs and the
validated boundary; it contains no source payload, content, email, phone or
secret. Partial evidence flags, evidence drift, duplicate or unexpected
lineage, protected-Place drift, later-phase lineage, a wrong first Offer, or an
existing output path all fail closed.

Checkpoint consumption uses a separate same-image tuple:
...
pnpm migration:phoenix-release \
  ...
  --manifest docs/migration/releases/phoenix-approved-2026-07-30.json \
  --plan \
  --continue-from-live-checkpoint <checkpoint path> \
  --continue-from-live-checkpoint-sha256 <exact SHA-256> \
  --continue-from-live-checkpoint-code-sha <generating code SHA>
```

The read-only form revalidates the complete live state and reports the exact
boundary. Replacing `--plan` with `--apply` is a separately authorized write:
the checkpoint code SHA must equal the running image SHA, Users, Businesses
and Places are not invoked, and execution starts at the first Offer. The three
flags are all-or-none and cannot combine with report continuation,
`--resume-from`, checkpoint creation or `--rerun`.
