...
- При включённом PROD runtime-конфиге `AnalyticsLoader` сначала создаёт `dataLayer`/официально совместимый `gtag`, затем ставит consent default: `analytics_storage`, `ad_storage`, `ad_user_data`, `ad_personalization` = `denied` и `wait_for_update=500`. Пауза даёт CookieConsent восстановить сохранённый выбор returning user до initial measurement; в consent update этот параметр не передаётся. Только после default добавляется `gtag.js`, вызываются `gtag("js", ...)` и один `gtag("config", ...)`.
